...
function scrape_insta_hash($tag) {
	...
	//print_r(array_values($insta_array));
...
$image_array = array(); // array to store images.
	for ($i=0; $i < $limit; $i++) {
		...
		$owner_array = $latest_array['owner'];
		$owner_data = 'OWNER '.$owner_array['id'].' ENDOWNER ';
		...
		$likes_data = ' COUNT-LIKES '.$likes_array['count'].' LIKESEND ';
		$comments_array = $latest_array['comments'];
		$comments_data = ' COUNT-COMMENTS '.$comments_array['count'].' COMMENTSEND ';

		//'LIKES_AND_COMMENTS'.$latest_array['likes']['comments'].' LC_END';
		//$image_data  = '<img src="'.$latest_array['display_src'].'">'; actual image and different sizes
		array_push($image_array, $owner_data, $image_data, $caption_data, $likes_data, $comments_data);
	}
